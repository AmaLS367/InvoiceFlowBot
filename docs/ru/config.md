...
| `MINDEE_API_KEY` | Ключ Mindee API для распознавания счетов | Ключ API из кабинета Mindee (https://platform.mindee.com/) | отсутствует, без значения бот не стартует |
| `MINDEE_MODEL_ID` | Идентификатор модели Mindee для обработки счетов | ID модели из платформы Mindee (например, `mindee/invoices/v4` или UUID пользовательской модели) | отсутствует, без значения бот не стартует |

При отсутствии `BOT_TOKEN` приложение выбросит исключение на старте.
